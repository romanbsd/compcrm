...
export type IqGetHandler = (stanza: Element) => Element | null | Promise<Element | null>;
...
export type IqDisposition = { kind: 'respond'; stanza: Element } | { kind: 'error' } | { kind: 'dispatch' };
 ...
 */
export async function dispositionForStanza(stanza: Element, onIqGet?: IqGetHandler): Promise<IqDisposition> {
  if (stanza.name === 'iq') {
...
export function createComponentSession(config: GatewayConfig, onIqGet?: IqGetHandler): XmppComponentSession {
  const maxPendingIqRequests = config.maxPendingIqRequests ?? DEFAULT_MAX_PENDING_IQ_REQUESTS;
  if (!Number.isSafeInteger(maxPendingIqRequests) || maxPendingIqRequests <= 0) {
  ...
  const createClient = (): ReturnType<typeof component> => {
    ...
    const middlewareClient = client as typeof client & {
      middleware: {
        use(handler: (context: { stanza: Element }, next: () => Promise<unknown>) => Promise<unknown>): unknown;
      };
    };
    middlewareClient.middleware.use(async (context, next) => {
      const stanza = context.stanza as Element;
      const type = String(stanza.attrs.type ?? '');
      if (stanza.name !== 'iq' || (type !== 'get' && type !== 'set')) return next();
      if (activeInboundIq >= maxPendingIqRequests) {
        return xml('error', { type: 'wait' }, xml('resource-constraint', { xmlns: STANZA_ERROR_NS }));
      }
      activeInboundIq++;
      try {
        const disposition = await dispositionForStanza(stanza, onIqGet);
        const response = disposition.kind === 'respond' ? disposition.stanza : buildIqError(stanza);
  ...
  const requestIq = (stanza: Element, options: IqRequestOptions = {}): Promise<Element> => {
    ...
    if (pendingIqRequests.size >= maxPendingIqRequests) {
      return Promise.reject(new Error(`Too many pending IQ requests (limit ${maxPendingIqRequests})`));
    }
