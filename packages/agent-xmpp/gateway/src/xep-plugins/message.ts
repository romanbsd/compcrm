...
export function buildOutboundStanza(req: OutboundDeliverRequest, fromJid: string): Element {
  ...
  const text =
    typeof req.content === 'string'
      ...
      : (req.content as { text?: string })?.text ||
        (typeof req.content === 'object' && req.content !== null ? JSON.stringify(req.content) : String(req.content));
  ...
  return xml('message', { type, id, to, from: fromJid, ...(req.lang ? { 'xml:lang': req.lang } : {}) }, ...children);
}

export function applyStoreHints(stanza: Element, policy?: MessagePolicy): Element {
  if (policy?.store === false) {
    return xml('message', stanza.attrs, ...stanza.children, xml('no-store', { xmlns: STORE_NS }));
  }
  if (policy?.store === true) {
    return xml('message', stanza.attrs, ...stanza.children, xml('store', { xmlns: STORE_NS }));
  }
  return stanza;
}
